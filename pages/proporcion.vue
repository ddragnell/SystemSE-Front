  ...
  <v-container class="container-p">
    ...
    >
      <v-card-title style="text-align: center"
        >Intervalo de confianza para la proporción</v-card-title
      >
      ...
      <div class="media">
        ...
        <v-text-field
          ...
          v-model="proporcion"
          density="compact"
          placeholder="Valor de la proporción muestral"
...
const camposCompletos = computed(() => {
  return confianza.value && proporcion.value && muestra.value;
});
...
const calcular = async () => {
  ...
  if (camposCompletos.value) {
    if (
      !utilsService.isNumber(proporcion.value) ||
      !utilsService.between0And1(proporcion.value)
    ) {
      ...
      if (interval) {
        alertService.informative(
          ...
          `Con una confianza del ${
            confianza.value * 100
          }%, se espera que la proporción poblacional esté entre: <br/><br/><b>${
            interval.L
